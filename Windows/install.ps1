...
Write-Host "Installing The Cube..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "The Cube.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "The Cube"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named The Cube. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching The Cube..."
