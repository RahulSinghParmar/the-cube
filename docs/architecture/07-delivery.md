...
| M4 | PLL then OLL/F2L practice, content provenance, attempt tracking, optional collection | Verified setup/algorithms, content QA and progress correctness | PLL first release implemented; remaining scope planned |
...
- **M4 (PLL first release implemented; remaining scope planned):** algorithm provenance catalog → setup/solution validator → PLL case viewer → recognition/execution attempt model → trainer UI → OLL/F2L sets → collection module if demand supports it.
