...
test('new interface and settings pass automated WCAG AA checks in each theme',async({page})=>{
  ...
  for(const theme of ['light','dark','contrast']){
    ...
    for(const route of ['#/settings','#/play','#/menu','#/algorithms','#/training','#/guides','#/statistics','#/about','#/license','#/learn','#/solve','#/train','#/recognize','#/oll']){
      await page.goto(`/the-cube/menu.html${route}`);
      await expect(page.locator('h1')).toBeVisible();
      if(route==='#/recognize') {
        await page.getByRole('button',{name:'Start recognition',exact:true}).click();
        const question=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21aa','wcag22aa']).analyze();expect(question.violations).toEqual([]);
